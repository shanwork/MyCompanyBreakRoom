...
var IMAGE_TOP_MARGIN = 2;
var IMAGE_BOTTOM_MARGIN = 2;
...
var ITEM_COUNT = 3 // item count in slots
...
function copySubArray(array, start, end) {
    var copy = [];
...
Game.prototype.restart = function () {
    $('#coffeeDispense').fadeOut();
    $('#teaDispense').fadeOut();
    $('#esspressoDispense').fadeOut();
    this.lastUpdate = new Date();
...
Game.prototype.update = function () {
    ...
    switch (this.state) {
        ...
        case 6: // check results
            var ec = 0;
            /*test 
            blinker('#coffeeDispense', 10);
            blinker('#teaDispense', 10);
             blinker('#esspressoDispense', 10);
       
        */
            var won = false;
            if (that.items1[that.result1].id.indexOf('coffee') >= 0
                    && that.items2[that.result2].id.indexOf('coffee') >= 0
                    && that.items3[that.result3].id.indexOf('coffee') >= 0) {
                blinker('#coffeeDispense', 2);
                won = true;
            }
            else if (that.items1[that.result1].id.indexOf('tea') >= 0// == 'tea-pot'
                    && that.items2[that.result2].id.indexOf('tea') >= 0// == 'tea-strainer'
                    && that.items3[that.result3].id.indexOf('tea') >= 0) {
                blinker('#teaDispense', 2);
                won = true;

            }
            else if (that.items1[that.result1].id.indexOf('espresso') >= 0// == 'espresso-machine'
                    && that.items2[that.result2].id.indexOf('espresso') >= 0// == 'espresso-tamper'
                    && that.items3[that.result3].id.indexOf('espresso') >= 0) {
                blinker('#esspressoDispense', 2);
                won = true;
            }
            if (won == true) {
                $('#slotStatus').text('WINNER!!');
                blinker('#slotStatus', 3);
